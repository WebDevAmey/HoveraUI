...
import NavbarDemo from "@/components/navbars/NavbarDemo";
...
export const navbarDoc: ComponentDocEntry = {
  slug: "navbar",
  name: "Navbar",
  description: "A fixed bottom navigation bar with spring-active pill indicator and glass backdrop.",
  category: "navbars",
  stack: classifyStack(NAVBAR_CODE),
  isNew: true,
  story:
    "A dock-style bottom nav pinned to the viewport. Clicking an icon springs the active pill indicator to that position via a shared layoutId. A subtle glass backdrop and separators between groups give it a polished app-dock feel.",
  Preview: NavbarDemo,
  code: NAVBAR_CODE,
  usage: `import Navbar from "@/components/ui/navbar"
...
export function AppNavigation() {
  ...
  props: [
    { name: "items", type: "NavItem[]", description: "Array of nav items. Each has icon, label, optional href/onClick, and optional separatorAfter." },
    { name: "defaultActive", type: "string", default: "items[0].label", description: "Label of the initially active item." },
    { name: "fixed", type: "boolean", default: "true", description: "Fix to the bottom of the viewport. Set false to render in-flow (e.g. inside preview containers)." },
    { name: "className", type: "string", default: '""', description: "Extra classes merged onto the outer wrapper." },
  ],
};
